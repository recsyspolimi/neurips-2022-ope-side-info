...
from obp.ope import SelfNormalizedInverseProbabilityWeighting, DirectMethod, DoublyRobust, SelfNormalizedDoublyRobust
...
def main(cfg: DictConfig) -> None:
    ...
    initial_evaluator = DeficientSupportInterpretableOPEEvaluator(
    ...
    k_cluster_list = list(range(5, 65, 5))

    for original_seed in np.arange(n_seeds):
        initial_evaluator.bandit_feedback = initial_evaluator._choose_bandit_feedback(s=original_seed)
        bootstrap_bandit_feedback, _, _, _, _ = initial_evaluator._sample_bootstrap_bandit_feedback_and_evaluation_policy(s=original_seed,
                                                                          sample_size=sample_size)
        original_n_actions = bootstrap_bandit_feedback['n_actions']
        original_def_support_rate = n_deficient_actions / original_n_actions
        original_pi_b = bootstrap_bandit_feedback['pi_b']
        ground_truth_original_pi_b = bootstrap_bandit_feedback['reward'].mean()

        original_n_rounds = bootstrap_bandit_feedback['n_rounds']


        cluster_est_list = [SelfNormInverseProbabilityWeightingClustering(
                                                       n_clusters=k,
                                                       action_context=dataset_ur.action_context[:, :-1]) for k in k_cluster_list]

        for i in range(len(cluster_est_list)):
            k = k_cluster_list[i]
            cluster_est_list[i].estimator_name= f"SN Sim. (cluster) {k}"



        # CROSS VALIDATION WITH DEFICIENT SUPPORT SIMULATED

        cross_val_evaluator = DeficientSupportInterpretableOPEEvaluator(
            random_states=np.array(cross_val_seeds),
            bandit_feedbacks=[bootstrap_bandit_feedback],
            evaluation_policies=[(ground_truth_original_pi_b, original_pi_b)],
            ope_estimators=cluster_est_list + [

                DiceSelfNormPureSimilarityIPS(estimator_name="SN Sim. (Dice)",
                                                 action_context=dataset_ur.action_context[:, :-1]),
                JaccardSelfNormPureSimilarityIPS(estimator_name="SN Sim. (Jaccard)",
                                                 action_context=dataset_ur.action_context[:, :-1]),
                SelfNormPureSimilarityIPS(estimator_name="SN Sim. (cosine) 2",
                                          action_context=dataset_ur.action_context[:, :-1]),
            ],
            regression_models=regression_models,
            n_deficient_actions=int(original_def_support_rate* (original_n_actions - n_deficient_actions)),
        )

        cross_val_evaluator.is_cross_validation = True

        # conduct an evaluation of OPE experiment
        logger.info("cross-validation started")
        _ = cross_val_evaluator.estimate_policy_value(sample_size=original_n_rounds, n_folds_=n_folds)
        # calculate statistics
        root_mse = cross_val_evaluator.calculate_mean(root=True)
        mean_scaled = cross_val_evaluator.calculate_mean(scale=True, root=True)
        ...
        cross_val_path = Path("./outputs/cross_val")
        cross_val_path.mkdir(exist_ok=True, parents=True)
        ...
        root_mse_df.to_csv(cross_val_path / f"cross_val_{original_seed}_root_mse.csv")

        # save mse
        mse = cross_val_evaluator.calculate_mean(root=False)

        mse_df = DataFrame()
        mse_df["estimator"] = list(mse.keys())
        mse_df["mse"] = list(mse.values())
        mse_df.to_csv(cross_val_path / f"cross_val_{original_seed}_mse.csv")


        se_df = DataFrame(cross_val_evaluator.calculate_squared_error())
        se_df = DataFrame(se_df.stack()).reset_index(1)
        se_df.rename(columns={"level_1": "estimators", 0: "se"}, inplace=True)

        res_dict = cross_val_evaluator.squared_error

        for key in res_dict.keys():
            if isinstance(res_dict[key], np.ndarray):
                res_dict[key] = res_dict[key].tolist()

        with open(cross_val_path / f"cross_validation_{original_seed}.json", "w") as fp:
            json.dump(res_dict, fp)

        experiment = f"{campaign}-{behavior_policy}-{sample_size}"
        elapsed_time = np.round((time.time() - start_time) / 60, 2)
        logger.info(f"finish cross val {experiment} seed {original_seed} in {elapsed_time}min")
